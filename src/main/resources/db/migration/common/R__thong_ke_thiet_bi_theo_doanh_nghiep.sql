drop function if exists thong_ke_thiet_bi_theo_doanh_nghiep(text, text, text, text, text);

create or replace function thong_ke_thiet_bi_theo_doanh_nghiep(dm_nhom_thiet_bi_ids text, dm_loai_thiet_bi_ids text, dm_don_vi_ids text,hang_san_xuat_ids text, dm_tinh_thanh_pho_ids text)
    returns TABLE(id bigint, ten character varying, so_luong bigint)
as
$$
begin
    return query
        select dv.id, dv.ten, count(tb.id) as so_luong
        from thiet_bi tb
