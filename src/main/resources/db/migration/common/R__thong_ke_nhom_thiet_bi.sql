drop function if exists thong_ke_nhom_thiet_bi(text, text, text, text, text);

create or replace function thong_ke_nhom_thiet_bi(dm_nhom_thiet_bi_ids text, dm_loai_thiet_bi_ids text, dm_don_vi_ids text,hang_san_xuat_ids text, dm_tinh_thanh_pho_ids text)
    returns TABLE(id bigint, ten character varying, so_luong bigint)
as
$$
begin
    return query
        select ntb.id, ntb.ten, count(tb.id) as so_luong
        from thiet_bi tb
