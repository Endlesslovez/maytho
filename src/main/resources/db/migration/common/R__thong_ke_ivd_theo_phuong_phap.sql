drop function if exists thong_ke_ivd_theo_phuong_phap(text, text, text);

create or replace function thong_ke_ivd_theo_phuong_phap(dm_phuong_phap_ids text, dm_don_vi_ids text,hang_san_xuat_ids text)
    returns TABLE(id bigint, ten character varying, so_luong bigint)
as
$$
begin
    return query
        select dppxn.id, dppxn.ten, count(ivd.id) as so_luong
