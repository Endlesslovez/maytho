drop function if exists thong_ke_ivd_theo_doanh_nghiep(text, text, text);

create or replace function thong_ke_ivd_theo_doanh_nghiep(dm_phuong_phap_ids text, dm_don_vi_ids text,hang_san_xuat_ids text)
    returns TABLE(id bigint, ten character varying, so_luong bigint)
as
$$
begin
    return query
        select dv.id, dv.ten, count(ivd.id) as so_luong
